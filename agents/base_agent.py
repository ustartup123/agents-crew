...
class BaseAgent:
    ...
    def run(self, prompt_text: str) -> dict:
        system_prompt = (
            ...
            f"CRITICAL RULES:\n"
            f"- Your backstory is fictional context for your persona. Do NOT treat it "
            f"as real company history or reference fictional products/deals from it.\n"
            f"- NEVER invent, fabricate, or hallucinate projects, products, metrics, "
            f"revenue numbers, customer names, or any data that was not provided to you.\n"
            f"- Only reference and work on the specific project described in your task below.\n"
            f"- When writing to Notion or Slack, only include real data from the task context.\n\n"
            f"Task: {prompt_text}\n\n"
